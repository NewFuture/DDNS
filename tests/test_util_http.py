...
class TestCreateConnection(unittest.TestCase):
    ...
    def test_create_https_connection_custom_ca_success(self, mock_logger, mock_ssl_context, mock_https_conn):
...
class TestLoadSystemCaCerts(unittest.TestCase):
    ...
    @patch("ddns.util.http.logger")
    def test_load_ca_certs_success(self, mock_logger, mock_isfile):
        """测试成功加载CA证书"""
        mock_context = Mock()
    ...
    @patch("ddns.util.http.os.path.isfile")
    @patch("ddns.util.http.logger")
    def test_load_ca_certs_no_files(self, mock_logger, mock_isfile):
        """测试没有找到CA证书文件"""
        mock_context = Mock()
