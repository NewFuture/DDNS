...
- 本文档适用�?DNSPod 国际版（dnspod.com�?
- 中国版（dnspod.cn）请参阅 [DNSPod 中国版配置指南](dnspod.md)

## 概述

DNSPod 国际版（dnspod.com）是面向全球用户的权�?DNS 解析服务，在海外地区广泛使用，支持动�?DNS 记录的创建与更新。本 DDNS 项目支持多种认证方式连接 DNSPod 国际版进行动�?DNS 记录管理�?
...
2. 进入"User Center" > "API Token"或访�?<https://www.dnspod.com/console/user/security>
3. 点击"Create Token"，填写描述，选择域名管理权限，完成创�?
...
```json
{
    ...
    "dns": "dnspod_com",                // 当前服务�?
    "id": "123456",                     // DNSPod 国际�?API Token ID
    "token": "YOUR_API_TOKEN",           // DNSPod 国际�?API Token Secret
...
`ttl` 参数指定 DNS 记录的生存时间（TTL），单位为秒。DNSPod 国际版支持的 TTL 范围�?1 �?604800 秒（�?7 天）。如果不设置，则使用默认值�?
...
- **域名未找�?*：确保域名已添加�?DNSPod 国际版账号，配置拼写无误，域名处于活跃状�?
- **记录创建失败**：检查子域名是否有冲突记录，TTL 设置合理，确认有修改权限
- **请求频率限制**：DNSPod �?API 调用频率限制，降低请求频�?
- **地区访问限制**：DNSPod 国际版在某些地区可能有访问限�?

## 支持与资�?

- [DNSPod 国际版产品文档](https://www.dnspod.com/docs/)
- [DNSPod 国际�?API 参考](https://www.dnspod.com/docs/index.html)
- [DNSPod 国际版控制台](https://www.dnspod.com/)
- [DNSPod 中国版配置指南](./dnspod.md)

> **建议**：推荐使�?API Token 方式，提升安全性与管理便捷性，避免使用邮箱密码方式。对于中国大陆用户，建议使用 [DNSPod 中国版](./dnspod.md)�?
