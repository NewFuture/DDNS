...
阿里云边缘安全加速（ESA）是阿里云提供的边缘安全加速服务，支持 DNS 记录的动态管理。本 DDNS 项目通过 AccessKey ID �?AccessKey Secret 更新 ESA DNS 记录�?
...
5. 确保账号具有边缘安全加�?(`AliyunESAFullAccess`) 的操作权�?
...
- **ESA站点查询权限 + ESA DNS记录管理权限**：精细化权限控制
  - `esa:ListSites`：查询站点列�?
  - `esa:ListRecords`：查�?DNS 记录
  - `esa:CreateRecord`：创�?DNS 记录
  - `esa:UpdateRecord`：更�?DNS 记录

可以�?[RAM控制台](https://ram.console.aliyun.com/policies) 中查看和配置权限�?
...
| endpoint| API 端点      | URL            | [参考下方](#endpoint)                           | `https://esa.cn-hangzhou.aliyuncs.com`         | 服务商参�?|
| ttl     | TTL 时间      | 整数（秒�?    | 30~86400                           | 1(自动)        | 服务商参�?|
| proxy   | 代理设置      | 数组           | [参考](../config/json.md#proxy)           | �?       | 公用网络   |
| ssl     | SSL 验证方式  | 布尔/字符�?   | `auto`、`true`、`false`            | `auto`    | 公用网络   |
| cache   | 缓存设置      | 布尔/字符�?   | `true`、`false`、`filepath`        | `true`    | 公用配置   |
| log     | 日志配置      | 对象           | [参考](../config/json.md#log)             | �?       | 公用配置   |
...
阿里�?ESA 支持多个区域端点，可根据区域和网络环境选择最优节点：
