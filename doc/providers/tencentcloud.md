# 腾讯�?DNS (TencentCloud DNSPod) 配置指南

## 概述

腾讯�?DNS（TencentCloud DNSPod）是腾讯云提供的专业 DNS 解析服务，具有高可用性和高性能，支持动�?DNS 记录的创建与更新。本 DDNS 项目通过 SecretId �?SecretKey 进行 API 认证�?
...
可以�?[访问管理控制台](https://console.cloud.tencent.com/cam/policy) 中查看和配置权限�?
...
| endpoint| API 端点      | URL            | [参考下方](#endpoint)              | `https://dnspod.tencentcloudapi.com` | 服务商参�?|
...
腾讯�?DNSPod API 支持多个区域端点，可根据网络环境选择最优节点：
...
> **注意**：建议使用默认端�?`https://dnspod.tencentcloudapi.com`，腾讯云会自动路由到最优节点。只有在特殊网络环境下才需要指定特定区域端点�?

### ttl

`ttl` 参数指定 DNS 记录的生存时间（TTL），单位为秒。腾讯云 DNSPod 支持�?TTL 范围�?1 �?604800 秒（�?7 天）。如果不设置，则使用默认值�?
...
> 参考：腾讯�?[云解�?TTL 说明](https://cloud.tencent.com/document/product/302/9072)
...
> 更多线路参考：腾讯�?[云解�?DNS 解析线路说明](https://cloud.tencent.com/document/product/302/8643)
>
## 故障排除
...
- **签名错误**：检�?SecretId �?SecretKey 是否正确，确认密钥没有过�?
- **域名未找�?*：确保域名已添加到腾讯云 DNSPod，配置拼写无误，域名处于活跃状�?
...
| 错误代码        | 说明             | 解决方案           |
| :------------- | :--------------- | :----------------- |
| AuthFailure.SignatureExpire | 签名过期 | 检查系统时�?      |
| AuthFailure.SecretIdNotFound | SecretId不存�?| 检查密钥配�?    |
| ResourceNotFound.NoDataOfRecord | 记录不存�?| 检查记录设�?    |
...
- [腾讯�?DNSPod 产品文档](https://cloud.tencent.com/document/product/1427)
- [腾讯�?DNSPod V3 API 文档](https://cloud.tencent.com/document/api/1427)
- [腾讯�?DNSPod 控制台](https://console.cloud.tencent.com/dnspod)
- [腾讯云技术支持](https://cloud.tencent.com/online-service)

> **建议**：推荐使用子账号 API 密钥并仅授予必要�?DNSPod 权限，以提高安全性。定期轮�?API 密钥确保账号安全�?
