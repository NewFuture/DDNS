...
华为�?DNS 是华为云提供的权�?DNS 解析服务，具有高可用性、高扩展性和高安全性，支持动�?DNS 记录的创建与更新。本 DDNS 项目通过 Access Key ID �?Secret Access Key 进行 API 认证�?

> ⚠️ **注意**：华为云 DNS Provider 目前处于**待验�?*状态，缺少充分的真实环境测试。请通过 [GitHub Issues](https://github.com/NewFuture/DDNS/issues) 反馈�?
...
使用华为�?Access Key ID �?Secret Access Key 进行认证�?
...
2. 进入 **我的凭证** > **访问密钥**
3. 点击"新增访问密钥"按钮
4. 复制生成�?**Access Key ID** �?**Secret Access Key**，请妥善保存
5. 确保账号具有云解�?DNS 的操作权�?

```json
{
    "dns": "huaweidns",
    "id": "your_access_key_id",     // 华为�?Access Key ID
    "token": "your_secret_access_key" // 华为�?Secret Access Key
}
```
...
可以�?[统一身份认证服务](https://console.huaweicloud.com/iam/) 中查看和配置权限�?
...
| :-----: | :----------- | :------------- | :--------------------------------- | :-------- | :--------- |
| dns     | 服务商标�?  | 字符�?        | `huaweidns`                        | �?       | 服务商参�?|
| id      | 认证 ID      | 字符�?        | 华为�?Access Key ID               | �?       | 服务商参�?|
| token   | 认证密钥     | 字符�?        | 华为�?Secret Access Key           | �?       | 服务商参�?|
...
| endpoint| API 端点      | URL            | [参考下方](#endpoint)              | `https://dns.myhuaweicloud.com` | 服务商参�?|
| line    | 解析线路      | 字符�?        | [参考下方](#line)                   | `default` | 服务商参�?|
| ttl     | TTL 时间      | 整数（秒�?    | 1~2147483647                   | `300`     | 服务商参�?|
...
华为�?DNS 支持多个区域端点，可根据区域和网络环境选择最优节点：

#### 国内节点

- **全球服务（推荐）**：`https://dns.myhuaweicloud.com`
- **华北-北京�?*：`https://dns.cn-north-4.myhuaweicloud.com`
...
> **注意**：建议使用默认端�?`https://dns.myhuaweicloud.com`，华为云会自动路由到最优节点。只有在特殊网络环境下才需要指定特定区域端点�?
...
- **认证失败**：检�?Access Key ID �?Secret Access Key 是否正确，确认密钥没有被删除或禁�?
- **域名未找�?*：确保域名已添加到华为云 DNS 解析，配置拼写无误，域名处于活跃状�?
...
| 错误代码        | 说明           | 解决方案           |
| :------------- | :------------- | :----------------- |
| APIGW.0301     | 认证失败       | 检查访问密�?      |
| DNS.0101       | 域名不存�?    | 检查域名配�?      |
| DNS.0102       | 记录集不存在   | 检查记录设�?      |
| DNS.0103       | 记录集已存在   | 检查冲突记�?      |
| DNS.0203       | 请求频率过高   | 降低请求频率       |

## 支持与资�?

- [华为�?DNS 产品文档](https://support.huaweicloud.com/dns/)
- [华为�?DNS API 文档](https://support.huaweicloud.com/api-dns/)
- [华为云控制台](https://console.huaweicloud.com/dns/)
- [华为云技术支持](https://support.huaweicloud.com/)

> ⚠️ **待验证状�?*：华为云 DNS Provider 缺少充分的真实环境测试，建议在生产环境使用前进行充分测试。如遇问题请通过 [GitHub Issues](https://github.com/NewFuture/DDNS/issues) 反馈�?
