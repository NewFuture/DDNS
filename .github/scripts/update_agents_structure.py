...
def write_issue_body(added, deleted, contract_errors, docs_changes):
    ...
    required_updates = []
    if contract_errors:
        required_updates.append("Fix each contract error above")
    if added or deleted:
        required_updates.extend(("Update directory structure", "Update version/date"))
    if docs_changes:
        required_updates.append("Update docs/llms.txt")
    lines.append("## Required Updates\n")
    lines.extend("%d. %s" % (index, update) for index, update in enumerate(required_updates, 1))
