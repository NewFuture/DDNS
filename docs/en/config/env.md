...
| `DDNS_CACHE_VERIFY_EVERY` | Non-negative integer                                                                                                                                   | Force one upstream verification after N consecutive cache hits       | `DDNS_CACHE_VERIFY_EVERY="10"`                                                                                                                    |
...
- **Required**: No
- **Default**: `0`
- **Description**: When the local IP stays the same, force one upstream verification on the `N+1` run after `N` consecutive cache hits
- **Examples**:

  ```bash
  # Disable forced verification
  export DDNS_CACHE_VERIFY_EVERY="0"

  # Force verification after 10 cached skips
