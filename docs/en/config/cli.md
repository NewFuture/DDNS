...
| `--cache-verify-every` |   Integer   | Force one upstream verification after N consecutive cache hits                                                                                                               | `--cache-verify-every 10`                                                                                                                 |
...
## Cache Verification Counter

### `--cache-verify-every COUNT`

Force periodic upstream verification based on consecutive cache hits.

- **Default**: `0`
- **Available values**:
  - `0`: Disabled, keep the current cache behavior
  - Positive integer: When the local IP does not change and the cache is hit `COUNT` times in a row, the `COUNT + 1` run forces an upstream verification
- **Examples**:
  - `--cache-verify-every 0` (disable)
  - `--cache-verify-every 10` (force verification after 10 cached skips)
