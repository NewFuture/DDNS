...
ESA Pages 自动缓存 `node_modules` 目录，加快后续构建速度。如需清除缓存：
...
ESA Pages 支持为每个 Pull Request 创建预览环境：

1. 在项目设置中启用 **"PR 预览"**
2. 每个 PR 会自动创建独立的预览 URL
3. 预览环境独立于生产环境，不会影响主站
...
2. 常见错误：
   ...
   - **VitePress 构建错误**：检查 Markdown 语法和链接
   - **死链检查失败**：修复文档中的无效链接
...
# 检查死链
npm run build  # VitePress 会自动检查死链
```
...
- **"EdgeKV namespace not found"**
  - 解决：创建命名空间 `ddns-releases`

- **"Failed to fetch from GitHub"**
  - 解决：检查网络连接，GitHub API 可能有速率限制

- **"404 Not Found"**
  - 解决：检查请求路径格式，确认文件在 GitHub Release 中存在
...
1. 检查域名是否已在 ESA 中添加并验证
2. 检查域名 DNS 解析是否正确指向 ESA
3. 检查 Pages 项目是否正确绑定自定义域名
...
```bash
# 检查 DNS 解析
nslookup ddns.newfuture.cc

# 检查 HTTP 访问
curl -I https://ddns.newfuture.cc

# 检查 HTTPS 证书
openssl s_client -connect ddns.newfuture.cc:443 -servername ddns.newfuture.cc
```
...
3. **调整缓存策略**：
   - 编辑 `esa.js` 中的 `Cache-Control` 头
   - 静态资源：`public, max-age=31536000, immutable`
   - 动态资源：`public, max-age=43200`（12 小时）
