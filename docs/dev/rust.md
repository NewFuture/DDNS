...
NameSilo 和 ClouDNS 仅在域名查询返回文档定义的未命中响应时尝试下一个候选域名；
鉴权、权限、HTTP 和畸形响应错误直接上报，不会被替换为“域名不存在”。
ClouDNS、华为云与 NameSilo 的记录查询会拒绝畸形集合或缺少匹配字段的记录，
避免误建重复记录；ClouDNS 的 `{}` 和 `[]` 均保留为空集合。
华为云查询签名与实际请求使用相同的 RFC3986 查询字符串，空格编码为 `%20`
而非 `+`，发送时不再二次表单编码；其他 Provider 的表单编码保持不变。

AliDNS 和 ESA 在提供 `extra` 时仍发送更新请求，即使地址和 TTL 没有变化；
未提供扩展参数时保留原有的跳过无变化更新行为。
...
`regex:` 从提取的原始地址开头匹配，保留 IPv6 的零填充、大小写和压缩写法，
例如 `^2001:0db8:` 可以匹配 `2001:0db8::1`；返回值仍为校验后的 `IpAddr`。
`local` 和数字索引先按地址族排除回环与未指定地址，再按原有顺序从 `0` 开始索引；
没有可用地址或索引越界时明确报错。私有地址和链路本地地址仍可选取。
