...
::: tip 可视化配置
可以使用[配置生成与校验工具](/config/studio)创建多服务商配置、定位错误并下载 `config.json`。凭据仅在浏览器本地处理，不会上传；在工具中填写的认证信息会完整写入导出的配置，如需避免写入文件，请改用环境变量或命令行参数。
:::
...
IPv4 和 IPv6 独立处理：一族被禁用或获取地址失败，不会阻止另一族更新。同步会继续尝试其余域名，但只要任一已启用的记录更新失败，单次运行的 CLI 命令就返回非零退出码，Web/MCP 则报告同步失败。启用缓存时，已成功的记录仍会保留缓存。
...
`public` 和 `url:` 规则只从成功的 HTTP 响应中提取地址；错误响应会继续回退到后续 API 或规则，不会使用错误页面里的 IP。IPv6 地址支持包含点分 IPv4 尾部的完整表示，例如 `2001:db8::192.0.2.10`。
...
缓存文件按整体 mtime 判断有效期，单位为秒，默认 259200（72 小时）。下一次运行时，`now - mtime >= cache_max_age` 或 mtime 超前系统时间超过 2 秒即视为过期。为兼容文件系统与系统时钟的精度差异，最多 2 秒的超前按缓存年龄为 0 处理；设置为 `0` 仍会在每次运行清空已有缓存。缓存仍为原有扁平 JSON，不保存每条记录时间戳，也不迁移。由于整个文件只有一个 mtime，任何缓存内容写入都会刷新所有记录的有效期；共享缓存文件的限制不变。
