...
- **特殊说明**: 普通项会照常按逗号或分号分割；当某一项以 `url:`、`regex:`、`cmd:` 或 `shell:` 开头时，从该前缀起的剩余内容会作为一个完整配置项保留，不再继续分割
- **示例**:
...
`DDNS_CACHE_MAX_AGE` 控制整个缓存文件的有效期（秒），在下一次运行时按文件 mtime 判断，边界值也算过期，mtime 超前系统时间超过 2 秒也算过期。为兼容文件系统与系统时钟的精度差异，最多 2 秒的超前按缓存年龄为 0 处理；`0` 仍会每次清空已有缓存。它不是 DNS TTL；缓存仍是扁平 JSON，任何内容写入都会刷新整个文件并影响所有记录，不进行迁移，共享缓存限制不变。
...
2. **特殊前缀规则**: 对于 `index4` 和 `index6` 参数：
   - 普通项仍会按逗号或分号分割；遇到以 `url:`、`regex:`、`cmd:` 或 `shell:` 开头的项后，从该前缀起的剩余内容会作为一个完整配置项保留
