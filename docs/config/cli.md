...
| `--cache-verify-every` |    整数     | 连续命中缓存多少次后强制向上游校验一次                                                                                                                                    | `--cache-verify-every 10`                                                                                                                   |
...
按连续命中缓存的次数，定期强制向上游校验。

- **默认值**: `0`
- **可选值**:
  - `0`: 关闭，保持当前缓存行为
  - 正整数: 本地IP未变化且连续命中缓存 `COUNT` 次后，第 `COUNT + 1` 次强制向上游校验
- **示例**:
  - `--cache-verify-every 0` (关闭)
  - `--cache-verify-every 10` (连续跳过 10 次后强制校验)
